...
public class RefreshTokenTests
{
    ...
    public void Constructor_CreatesTokenWithCorrectProperties()
    {
        ...
        var token = new RefreshToken(userId, "test_token", TimeSpan.FromDays(7));
    ...
    {
        var token = new RefreshToken(Guid.NewGuid(), "test_token", TimeSpan.FromDays(7));
    ...
    {
        var token = new RefreshToken(Guid.NewGuid(), "test_token", TimeSpan.Zero);
    ...
    {
        Assert.Throws<EntityException>(() => new RefreshToken(Guid.Empty, "test_token", TimeSpan.FromDays(7)));
    }

    [Fact]
    public void Constructor_ThrowsForNullToken()
    {
        Assert.Throws<EntityException>(() => new RefreshToken(Guid.NewGuid(), null, TimeSpan.FromDays(7)));
    }

    [Fact]
    public void Constructor_ThrowsForEmptyToken()
    {
        Assert.Throws<EntityException>(() => new RefreshToken(Guid.NewGuid(), "", TimeSpan.FromDays(7)));
    }
}
